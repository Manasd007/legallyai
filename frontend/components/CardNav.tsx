...
const CardNav: React.FC<CardNavProps> = ({
  ...
  const glassBase = /^#[0-9a-fA-F]{6}$/.test(resolvedBase ?? "")
    ? `${resolvedBase}d9`
    : resolvedBase;
  const resolvedMenu = isDark ? menuColorDark ?? menuColor : menuColor;
  const resolvedButtonBg = isDark ? buttonBgColorDark ?? buttonBgColor : buttonBgColor;
  const resolvedButtonText = isDark ? buttonTextColorDark ?? buttonTextColor : buttonTextColor;
  ...
  return (
    <div
      className={`card-nav-container fixed left-1/2 -translate-x-1/2 w-[90%] max-w-[800px] z-[99] top-[1.2em] md:top-[2em] transition-transform duration-500 ease-out ${
            ...
            >
              ...
              <div className="nav-card-links mt-auto flex flex-col gap-[2px]">
                {item.links?.map((lnk, i) => (
                  ...
                  >
                    <ArrowUpRight className="nav-card-link-icon shrink-0 h-[18px] w-[18px]" aria-hidden="true" />
                    {lnk.label}
                  </Link>
                ))}
              </div>
